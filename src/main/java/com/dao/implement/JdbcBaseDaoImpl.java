...
public abstract class JdbcBaseDaoImpl<T> implements JdbcBaseDao<T> {
    ...
    public void delete(String id) {
        String sql = " DELETE FROM " + entityClass.getSimpleName() + " WHERE rowguid=?";
            ...
            if (fields != null && fields.length > 0) {
                ...
                for (Field field : fields) {
                    try {
                        field.setAccessible(true);
                        Object obj = field.get(t);
                        if (field.getName().equalsIgnoreCase("rowguid")) {
                            primaryKey = obj;
                            id = obj;
                        } else {
                            if (params.size() == 0) {
    ...
    public T findById(String id) {
        String sql = "SELECT * FROM " + entityClass.getSimpleName() + " WHERE rowguid=?";
    ...
    public String makeSql(String sqlFlag) {
        ...
        if (sqlFlag.equals(SQL_INSERT)) {
            ...
            for (int i = 0; fields != null && i < fields.length; i++) {
                fields[i].setAccessible(true); // 暴力反射
                String column = fields[i].getName();
                if (column.equals("rowguid")) { // id 代表主键
                    continue;
                }
                sql.append(column).append("=").append("?,");
            }
            sql = sql.deleteCharAt(sql.length() - 1);
            sql.append(" WHERE rowguid=?");
        } else if (sqlFlag.equals(SQL_DELETE)) {
            sql.append(" DELETE FROM " + entityClass.getSimpleName() + " WHERE rowguid=?");
        }
    ...
    public void batchDelete(String[] ids) {
        ...
        String sql = " DELETE FROM " + entityClass.getSimpleName() + " WHERE rowguid in (?)";
